...
export const AuthStatusCard: React.FC<AuthStatusCardProps> = ({
  ...
  const eyebrowClassName = tone === 'warning' ? 'text-amber-700' : 'text-teal-700';

  return (
    <div className={`rounded-3xl ${toneClassName} p-8 shadow-sm backdrop-blur`}>
      <div className="flex items-start gap-4">
        ...
        <div>
          ...
          <p className="mt-1 text-3xl font-semibold tracking-tight text-slate-950">{title}</p>
...
export const AuthSplitLayout: React.FC<AuthSplitLayoutProps> = ({
  ...
  <div className="min-h-screen bg-transparent lg:grid lg:grid-cols-[minmax(0,1fr)_minmax(420px,46vw)]">
    <section className="flex min-h-screen items-center justify-center px-6 py-12 sm:px-8 lg:px-12 xl:px-16">
      <div className="w-full max-w-md">
        <div className="mb-10 flex items-center gap-4">
          <Logo size="sm" showText={false} />
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-teal-700">Golf Handicap System</p>
        ...
        <div>
          ...
          {introDetail ? <p className="mt-2 text-sm leading-6 text-slate-500">{introDetail}</p> : null}
        </div>

        <div className="mt-10">{children}</div>

        {footer ? <div className="mt-8 text-sm leading-6 text-slate-600">{footer}</div> : null}
      </div>
    </section>
